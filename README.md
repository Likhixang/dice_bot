...
#### 连胜/连败奖惩
- **乐善好施**：连赢 3 局（有积分加）→ 按最近 3 局平均下注的 **50% 四舍五入取整** 自动扣分，重置后继续循环计算
- **同舟共济**：连败 3 局（有积分扣）→ 按最近 3 局平均下注的 **50% 四舍五入取整** 自动补贴，重置后继续循环计算
- 平局（±0）重置连胜/连败计数；胜败以实际盈亏符号判定，与名次无关

#### 极端点数奖惩
- **比大出 0 点 / 比小出 9 点**：太惨了！系统补偿该局下注的 **50% 四舍五入取整**
- **比大出 9 点 / 比小出 0 点**：太幸运了！回馈社会该局下注的 **50% 四舍五入取整**
